...
const signup= async (req,res)=>{
    try{
        ...
        if(userExits.length>0){
            ...
            let user= new User({first_name,last_name, email, mobile, password, role_type})
            await user.save();
              res.status(200).send({
                status: "success",
...
const signin= async (req,res)=>{
    try{
        ...
        if(user_detail){
            ...
            let token =await jwt.sign({ user_detail: user_detail }, process.env.Secret, {expiresIn: 86400 }); // expires in 24 hours
...
const allAuthors= async (req,res)=>{
    try{
        const userdata= await User.find({role_type: "Author"})
